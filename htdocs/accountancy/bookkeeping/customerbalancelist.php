...
if ($action != 'export_csv')
{
	...
	print_liste_field_titre("Subledger Account Number", $_SERVER['PHP_SELF'], "", "", $param, "", $sortfield, $sortorder);
	print_liste_field_titre("Customer Name", $_SERVER['PHP_SELF'], "", "", $param, "", $sortfield, $sortorder);
	...
	// fetch code_compta, nom, opening_balance given search date start
	$sql = "select ".MAIN_DB_PREFIX."societe.code_compta, (SUM(t.debit) - SUM(t.credit)) as opening_balance from ".MAIN_DB_PREFIX."accounting_bookkeeping as t inner join ".MAIN_DB_PREFIX."societe on t.subledger_account = ".MAIN_DB_PREFIX."societe.code_compta and t.doc_date < '".$db->idate($search_date_start)."' where t.entity in (1) group by ".MAIN_DB_PREFIX."societe.code_compta";
	$resql = $db->query($sql);
	$nrows = $resql->num_rows;
	$opening_balance_array = $resql->fetch_all();
	$subledger_opening_balance = array();
	foreach($opening_balance_array as $row) {
		$subledger_opening_balance["{$row[0]}"] = $row[1];
	}

	// fetch code_compta, nom, debit, credit for search date range
	$sql = "select ".MAIN_DB_PREFIX."societe.code_compta, MAX(".MAIN_DB_PREFIX."societe.nom) as nom, SUM(t.debit) as debit, SUM(t.credit) as credit from ".MAIN_DB_PREFIX."accounting_bookkeeping as t inner join ".MAIN_DB_PREFIX."societe on t.subledger_account = ".MAIN_DB_PREFIX."societe.code_compta and t.doc_date between '".$db->idate($search_date_start)."' and '".$db->idate($search_date_end)."' where t.entity in (1) group by ".MAIN_DB_PREFIX."societe.code_compta";
	$resql = $db->query($sql);
	$nrows = $resql->num_rows;
	...
	for($i = 0; $i < count($debit_credit_array); $i++)
	{
		...
		print '<td>'.length_accountg($subledger_account).'</td>';
		print '<td>'.$customer_name.'</td>';
		print '<td class="nowraponall right">'.price($opening_balance).'</td>';
		print '<td class="nowraponall right">'.price($debit).'</td>';
		print '<td class="nowraponall right">'.price($credit).'</td>';
		print '<td class="nowraponall right">'.price($opening_balance + $debit - $credit).'</td>';
		print '<td class="center">'.$link;
		print '</td>';
		print "</tr>\n";

		// Comptabilise le sous-total
		$sous_total_debit += $debit;//$line->debit;
		$sous_total_credit += $credit;//$line->credit;
