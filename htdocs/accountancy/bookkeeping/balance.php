...
if ($action != 'export_csv')
{
	...
	print '<td class="liste_titre" colspan="6">'; //MHA 5->6
	...
	print_liste_field_titre("Opening Balance", $_SERVER['PHP_SELF'], "t.label_operation", "", $param, 'class="right"', $sortfield, $sortorder);//MHA
	...
	//MHA
	$querystr = "select t.numero_compte, (SUM(t.debit) - SUM(t.credit)) as opening_balance from llx_accounting_bookkeeping as t where entity in (1) and t.doc_date < "
				."'".$db->idate($search_date_start)."' group by t.numero_compte order by t.numero_compte asc";
	...
	foreach ($object->lines as $line)
	{
		$opening_balance = $opening_balances["'".$line->numero_compte."'"];
		...
		if (empty($displayed_account) || $root_account_description != $displayed_account)
		{
			...
			//MHA removed as requested // Affiche le compte comptable en debut de ligne
			//print "<tr>";
